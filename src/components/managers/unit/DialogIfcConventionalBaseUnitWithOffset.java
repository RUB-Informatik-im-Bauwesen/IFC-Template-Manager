 ...
 *
 */
public class DialogIfcConventionalBaseUnitWithOffset extends Dialog<Object[]> {
	...
	public DialogIfcConventionalBaseUnitWithOffset(IfcModel model, Collection<IfcUnit> units) {
		this.setTitle("Creating IfcConventionalBaseUnitWithOffset");
		...
		grid.add(new Label("IfcReal Offset:"), 0, 3);
		grid.add(numberField, 1, 3);
		grid.add(new Label("Conversion Unit:"), 0, 4);
		grid.add(comboboxUnits, 1, 4);
		grid.add(new Label("UnitEnum:"), 0, 5);
		grid.add(comboboxC, 1, 5);
		grid.add(new Label("Dim.Expo.:"), 0, 6);
		grid.add(dimExpoButton, 1, 6);
		grid.add(dimExpoLbl, 1, 7);
		...
		*/
		...
		this.setResultConverter(dialogButton -> {
			if (dialogButton == createButtonType) {
				Object[] arr = {textFieldName.getText(), comboboxValueType.getValue(), textFieldValue.getText(), comboboxUnits.getValue(), comboboxC.getValue(), numberField.getText(), dimExpo};
				return arr;
			}
			return null;
		});
	...
	public IfcUnit getUnit(Object[] content) {
		IfcConversionBasedUnitWithOffset ifcUnit = new IfcConversionBasedUnitWithOffset.Ifc4.Instance();
		...
		ifcUnit.setConversionOffset(new IfcReal.Ifc4(Double.parseDouble((String)content[5])));
		IfcDimensionalExponents de = (IfcDimensionalExponents)content[6];
