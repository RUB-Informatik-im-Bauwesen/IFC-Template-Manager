...
import extensions.coloring.ColorSchema;
import extensions.coloring.ColorSchemaManager;
...
public class IfcSelectionSetViewFX extends JPanel {
		...
		public void mouseClicked(MouseEvent e) {
			if (SwingUtilities.isRightMouseButton(e)) {
				...
				if (selecteditems.size() == 1) {
					popupMenu = new IfcSelectionSetPopupMenuFX("SelectionSet PopUp", selecteditems.get(0));
				} else {
					popupMenu = new IfcSelectionSetPopupMenuFX("SelectionSet PopUp", selecteditems);
				}
	...
	private void initToolChain() throws IOException {
		...
		java.awt.Image image = ResourceManager.getIcon("del.png").getImage(); // transform it
		java.awt.Image newimg = image.getScaledInstance(16, 16, java.awt.Image.SCALE_SMOOTH); // scale it the smooth way


		JButton removeButton = ApplicationUtilities.createButton("Remove Selection Set", new ActionListener() {
		...
		image = ResourceManager.getIcon("add.png").getImage(); // transform it
		newimg = image.getScaledInstance(16, 16, java.awt.Image.SCALE_SMOOTH); // scale it the smooth way

		toolChainBottom.add(ApplicationUtilities.createButton("Add Selection Set", new ActionListener() {
		...
		toolChainTop.add(ApplicationUtilities.createButton("Edit Color Schema", new ActionListener() {
		...
		toolChainTop.add(ApplicationUtilities.createButton("Reset Color Schema", new ActionListener() {
					...
					protected void updateItem(Object item, boolean empty) {
						...
						if (item != null && !empty) {
							...
							if (item instanceof SelectionSet) {
								Image image = new Image(ResourceManager.getResource("icons/folder.png"), 16, 16, false, true);
